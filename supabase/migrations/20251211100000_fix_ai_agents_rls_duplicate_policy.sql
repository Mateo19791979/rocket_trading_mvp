-- Location: supabase/migrations/20251211100000_fix_ai_agents_rls_duplicate_policy.sql
-- Schema Analysis: RLS policy collision detected, fixing with DROP IF EXISTS pattern
-- Integration Type: Corrective - Fixing existing policy conflicts
-- Dependencies: Existing orchestration tables, auth system

-- 1. SAFE POLICY CLEANUP - Remove duplicates before recreation
DROP POLICY IF EXISTS "users_manage_own_user_profiles" ON public.user_profiles;

-- 2. TYPES - Agent status and categories for French system (Safe with IF NOT EXISTS)
DO $$
BEGIN
...
-- 3. CORE TABLES - Only create if not exists
...
-- 4. ASSETS TABLE - Required for trading operations
...
-- 5. AI AGENTS TABLE - Core agent management with French categories
...
-- 6. TRADES TABLE - Trading operations for agent tracking
...
-- 7. AI AGENT TRADES TABLE - Links agents to their trades
...
-- 8. SYSTEM HEALTH TABLE - Agent health monitoring
...
-- 9. ESSENTIAL INDEXES - Only create if not exists
...
-- 10. HELPER FUNCTIONS (Safe recreation with OR REPLACE)
...
-- 12. SAFE RLS POLICIES (Drop existing before recreation)
...
DROP POLICY IF EXISTS "users_manage_own_ai_agents" ON public.ai_agents;
DROP POLICY IF EXISTS "users_manage_own_trades" ON public.trades;
DROP POLICY IF EXISTS "users_can_view_agent_trades" ON public.ai_agent_trades;
DROP POLICY IF EXISTS "users_can_create_agent_trades" ON public.ai_agent_trades;
DROP POLICY IF EXISTS "users_can_view_agent_health" ON public.system_health;
DROP POLICY IF EXISTS "users_can_update_agent_health" ON public.system_health;
...
-- 13. SAFE TRIGGERS (Drop existing before recreation)
DO $$ 
BEGIN
...
-- 14. MOCK DATA FOR FRENCH AI AGENT SYSTEM (24 IA agents)
DO $$
DECLARE
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'demo@trading-mvp.fr') THEN
    ...
    ELSE
        ...
        SELECT id INTO demo_user_id FROM auth.users WHERE email = 'demo@trading-mvp.fr';
        SELECT id INTO orchestration_user_id FROM auth.users WHERE email = 'admin@trading-mvp.fr';
    END IF;
    ...
    DELETE FROM public.ai_agents WHERE user_id IN (demo_user_id, orchestration_user_id);
    ...
    RAISE NOTICE 'Migration réussie : 24 agents IA créés avec succès';
    RAISE NOTICE 'Identifiants de connexion demo : demo@trading-mvp.fr / demo123, admin@trading-mvp.fr / admin123';
